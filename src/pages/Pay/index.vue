...
export default {
  ...
  methods: {
    async getcreateNative() {
      let result = await this.$API.reqcreateNative(this.$route.query.trader);
      if (result.code == 200) {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "快点给老子付钱", {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          let result = await this.$API.reqPayStatus(this.$route.query.trader);
          if (result.code == 200) {
            clearInterval(this.timer);
            this.timer = null;
