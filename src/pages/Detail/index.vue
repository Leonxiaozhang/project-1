        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spusa, index) in spuSaleAttrList" :key="spusa.id">
                <dt class="title">{{ spusa.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: attr.isChecked == 1 }"
                  v-for="(attr, index) in spusa.spuSaleAttrValueList"
                  :key="attr.id"
                  @click="xuanzeebanben(spusa.spuSaleAttrValueList,attr)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skunum" @change="chengeskunum"/>
                <a href="javascript:" class="plus" @click="skunum++">+</a>
                <a href="javascript:" class="mins" @click="skunum>1?skunum--:skunum=1">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="upcart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getgoodsinfo", this.$route.params.hehe);
  },
  methods:{
  xuanzeebanben(a,b){
   a.forEach(item => {
  ...
  async upcart(){
    try {
      await this.$store.dispatch('getaddcart',{skuId:this.$route.params.hehe,skunum:this.skunum})
      this.$router.push({path: '/AddCartSuccess',query:{skunum: this.skunum}})
      sessionStorage.setItem('hehehe',JSON.stringify(this.skuInfo))
    } catch (error) {
      alert(error.message)
    }
  }
